...
6. Choose a preset and click **Build Staged Pack**. The before/after gallery opens automatically when the staged output is ready.
7. Inspect several rock, ground, structure, vegetation, and effect textures. Start with **Fit**, then choose **1:1** to judge the actual output pixels. You can reopen the gallery with **Open Preview Gallery**.
8. Open **Manage Staged Packs**. Check every completed pack you want active (for example, a repaired **Characters + Equipment** pack and **Zone - hateplane**), then choose **Install Checked Packs**. SpinTexture verifies and composes disjoint packs without running AI again. Close EverQuest and LaunchPad before installing.
...
Every completed build remains in the install profile's staged-pack library. The library shows its scope, preset, date, archive count, size, integrity state, archive contents, and available before/after gallery. Checkboxes control which completed packs are installed together. Packs with disjoint archive paths are composed by hard link when possible, so combining a large character pack with one or more zone packs does not rerun the upscaler or duplicate the staged payload. Conflicting versions of the same complete archive are blocked instead of silently choosing one.
...
- **Faithful** uses the conservative Real-ESRNet model. It cleans scaling/compression artifacts with minimal invented detail, so its improvement can be subtle at normal viewing size.
...
- **Maximum Detail** uses the strongest reconstruction route, passes through the same fidelity checks, and should still be reviewed zone by zone.
...
- Skips normal/data maps, numbered masks, UI/glyph/effect assets (including compact legacy spell/smoke names), packed palettes/atlases, unusual arrays/cubemaps, premultiplied DXT2/DXT4, unsafe formats, tiny textures, and likely sprite strips.
...
Managed builds and backups live under `%LOCALAPPDATA%\SpinTexture\Profiles\<install-name>-<hash>`. They are separate from the EverQuest directory.
