...
- **Graphic Painted Fantasy:** a detail-preserving neural reconstruction (PBRify SPAN when available, Real-ESRGAN x4plus as the legacy base) followed by SpinTexture's deterministic multi-pass painterly stylization — structure-tensor-oriented anisotropic sector filtering (coarse underpainting plus a fine pass), contrast-masked detail preservation, luminance detail re-injection, flow-aligned stroke grain, low-frequency hue/value jitter, and subtle canvas grain — then one concrete Classic Painted, Light Storybook, Dark Gothic, or Comic Ink palette treatment. Five recorded style sliders parameterize the stylization. Every sampler and noise lattice follows the wrap-or-clamp sampling policy and is periodic with the image, so tiling world materials stay seamless; alpha remains byte-exact, unsafe renderer assets stay protected, and failed outputs fall back safely. The reviewed zone map is an exact allowlist with Classic fallback. It cannot add mesh outlines or replace the game renderer.
